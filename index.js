			...
			Promise.all(files.map(function (filename) {
				return new Promise(function (resolve, reject) {
					fs.readFile(path.join(paths.cert, filename), 'utf8', function (err, data) {
						...
						config["certs"][filename] = Buffer.from(data).toString('base64');
						resolve();
					});
				});
			})).then(function () {
					...
					Promise.all(files.map(function (filename) {
						return new Promise(function (resolve, reject) {
							fs.readFile(path.join(paths.machine, filename), 'utf8', function (err, data) {
								...
								config["machines"][program.export][filename] = Buffer.from(data).toString('base64');
								resolve();
							});
						});
					})).then(function () {
...
if (program.import) {
	...
	Object.keys(input['machines']).map(function (machinename) {
		...
		Promise.all(Object.keys(input['certs']).map(function (filename) {
			return new Promise(function (resolve, reject) {
				fs.writeFile(path.join(paths.cert, filename), Buffer.from(input['certs'][filename], 'base64'), {
					'encoding': 'utf8',
					'mode': '600'
				}, function (err) {
					if (err) {
		...
		Promise.all(Object.keys(input['machines'][machinename]).map(function (filename) {
			return new Promise(function (resolve, reject) {
				var cleartext = Buffer.from(input['machines'][machinename][filename], 'base64');
